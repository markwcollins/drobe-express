...
import { IProfile, SupabaseTables } from '../types/supabase-types'
...
export default class UpdateWebPagesCron {
  ...
  async updateWebPages({ from, to }: IupdateWebPagesProps) {
    ...
    if (profileError) {
      consoleError(profileError)
      return false
    }
    if (!profileData ||!profileData.length) return false

    const profile = profileData[0]

    const { data: products, error: productsError } = await Product.selectAndPopulate({ userId: profile.user_id })
    if (productsError) {
      consoleError(profileError)
      return false
    }
    ...
    return await Promise.allSettled(products.map(async (productData) => {
      try {
        ...
        if (!WebPage.isValid(webPage)) throw new Error('webPage.isValid is not missing')
